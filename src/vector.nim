...
proc initVector*[T](length: int = DEFAULT_VECTOR_LENGTH,
    maxLength: int = UNLIMITED_LENGTH): Vector[T] =
  if 0 == T.sizeof():
  ...
  let memory = T.createU(toInt(toFloat(max(MINIMUM_VECTOR_LENGTH, length) *
      T.sizeof()).log(2).ceil()))
  if memory.isNil():
    raise newException(OutOfMemDefect, fmt"failed to allocate vector's memory of size {length}")

  Vector[T](memory: memory, amount: 0, size: length * T.sizeof(),
      maxAmount: maxLength, alive: true)
...
template getPtr*[T](vec: Vector[T], index: int): ptr T =
  if index >= vec.amount:
    raise newException(IndexDefect, "vector index out of range")
...
proc reserveSlot[T](vec: var Vector[T]): ptr T =
  if UNLIMITED_LENGTH != vec.maxAmount and vec.amount + 1 > vec.maxAmount:
    raise newException(OverflowDefect, "cannot push more than the max length")
  ...
  if (vec.amount + 1) * T.sizeof() > vec.size:
    ...
    if newMemory.isNil():
      raise newException(OutOfMemDefect,
          fmt"failed to reallocate vector's memory of size {vec.size}")
...
proc resize[T](vec: var Vector[T], size: int) =
  ...
  let newMemory = vec.memory.resize(vec.size)
  if newMemory.isNil():
    raise newException(OutOfMemDefect, fmt"failed to reallocate vector's memory of size {vec.size}")
  vec.memory = newMemory
...
proc extend*[T](vec: var Vector[T], items: seq[T]) =
  ...
  if UNLIMITED_LENGTH != vec.maxAmount and vec.amount + itemCount > vec.maxAmount:
    raise newException(OverflowDefect, "cannot extend more than the max length")
...
iterator mpairs*[T](vec: Vector[T]): var tuple[key: int, val: T] {.
    noSideEffect.} =
  for i in 0..<vec.amount:
    yield (i, vec[i])
